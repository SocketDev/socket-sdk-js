 ...
 *   Usage: node scripts/fleet/gen/llms-txt.mts [--check] [--no-ai] [--quiet]
 */
  ...
  try {
    ...
    if (checkOnly) {
      if (!existsSync(outPath)) {
        logger.error(
          'llms.txt is missing — run: node scripts/fleet/gen/llms-txt.mts',
        )
        process.exitCode = 1
        return
      }
      ...
      if (!structuresMatch(currentStruct, newStruct)) {
        logger.error(
          'llms.txt is stale — run: node scripts/fleet/gen/llms-txt.mts',
        )
        process.exitCode = 1
        return
      }
    ...
    logger.error(`gen/llms-txt failed: ${errorMessage(e)}`)
    process.exitCode = 1
  }
}
