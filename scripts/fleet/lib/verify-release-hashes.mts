 ...
 * report; the message is already fail-loud (What / Where / Saw-vs-wanted / Fix).
 */
 ...
 * and for the pre-approve path (which supplies the staged shasum from
 * `pnpm stage list`, since a staged version is not yet in the public packument).
 */
export async function verifyReleaseHashes(
  ...
  const fetchGitHub = opts.fetchGitHubAssetDigest ?? defaultFetchGitHubAssetDigest
...
async function defaultFetchGitHubAssetDigest(
  ...
  const result = await spawn(
    ...
    { cwd: opts.cwd, shell: WIN32, stdio: ['ignore', 'pipe', 'pipe'], stdioString: true },
