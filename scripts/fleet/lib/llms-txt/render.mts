...
function renderFacts(facts: RepoFacts): string {
  ...
  if (facts.version !== undefined) {
    rows.push(`| version | ${facts.version} |`)
  }
  if (facts.nodeFloor !== undefined) {
    rows.push(`| node | ${facts.nodeFloor} |`)
  }
  rows.push(`| layout | ${facts.layout} |`)
  if (facts.license !== undefined) {
    rows.push(`| license | ${facts.license} |`)
  }
...
export function structuresMatch(a: LlmsStructure, b: LlmsStructure): boolean {
  ...
  if (a.sectionTitles.length !== b.sectionTitles.length) {
    return false
  }
  for (let i = 0; i < a.sectionTitles.length; i += 1) {
    if (a.sectionTitles[i] !== b.sectionTitles[i]) {
      return false
    }
