/**
 * @file Gap engine — committed-tree secret scan (TruffleHog).
 *   Pure functions, no FS reads, no spawn, no network. The doctor resolves the
 *   FLEET's integrity-pinned, per-platform TruffleHog binary (downloaded +
 ...
 *   NEVER auto-fixed — rotation + history purge is a human decision.
 *
 *   Edit-time guards (secret-content-guard, no-token-in-dotenv-guard,
 *   token-guard) cover the write path; this is the missing committed-state scan.
 */
...
export function formatSecretFindings(
  ...
  return sortHits(hits).map(hit => {
    ...
    return {
      ...
      wanted: 'no credentials in tracked files (secrets live in the keychain / CI secret store)',
...
export function formatToolMissingFinding(): DoctorFinding {
  return {
    ...
    wanted: 'the fleet-pinned TruffleHog binary installed by setup-security-tools',
    what: 'Secret scan skipped: fleet TruffleHog unavailable',
    where: '.claude/hooks/fleet/setup-security-tools/external-tools.json (trufflehog)',
  }
}
