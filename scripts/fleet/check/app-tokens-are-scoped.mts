 ...
 *   radius instead of just what the job needs.
 *
 *   The minter is a `run:` step, not a recognizable third-party action, so
 *   zizmor's `github-app` audit does NOT see it — this check is the SOLE
 *   enforcement of the scope contract (it formerly backstopped zizmor's check of
 *   the `permission-*` inputs on the pinned token action).
 *
 *   Scans `.github/**` (workflows + composite actions) of the repo, the cascaded
 *   `template/base/.github/**`, and the fleet-shared override actions under
 *   `template/overrides/socket-registry/.github/**` (which host the shared
 *   release-/pr-app-token actions every member consumes). Exit 0 = every minter
 *   step scoped (or none used). Exit 1 = an unscoped step, listed with What /
 *   Where / Saw-vs-wanted / Fix. CI gate via `scripts/check.mts`.
 *   Usage: node scripts/fleet/check/app-tokens-are-scoped.mts.
 */
