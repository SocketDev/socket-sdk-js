...
/**
 * A per-file escape for the line cap: `doctrine: allow max-file-lines --
 * <reason>` within the first ten lines. The reason is mandatory — a bare pragma
 * does not match — because the exemption is a documented judgement, not a
 * switch. Meant for files that are first-party but only compile inside the full
 * Node build, where a blind split cannot be verified.
 */
export function hasLineCapPragma(lines: readonly string[]): boolean {
  const head = lines.slice(0, 10)
  for (let i = 0, { length } = head; i < length; i += 1) {
    // require-regex-comment: `doctrine: allow max-file-lines -- <reason>` — the
    // literal marker, then ` -- `, then at least one non-space reason char.
    if (/doctrine:\s*allow\s+max-file-lines\s+--\s+\S/.test(head[i]!)) {
      return true
    }
  }
  return false
...
/**
 * True when the file declares itself generated in its first five lines —
 * `auto-generated`, `@generated`, or `do not edit` markers. A generator's
 * output is sized by its input data, so the hand-written-source line cap does
 * not apply; every other doctrine rule still does.
 */
export function isGeneratedSource(lines: readonly string[]): boolean {
  const head = lines.slice(0, 5)
  for (let i = 0, { length } = head; i < length; i += 1) {
    // require-regex-comment: generated-file markers — `auto-generated` or
    // `autogenerated`, `@generated`, or `do not edit` / `do not hand-edit`.
    if (/auto-?generated|@generated|do not (?:hand-)?edit/i.test(head[i]!)) {
      return true
    }
  }
  return false
...
/**
 * True for vendored upstream trees staged INSIDE a patched-source overlay —
 * any path with a `source-patched/deps/` segment pair. Those files are copies
 * of pinned upstreams (the overlay stages them so the patched build can
 * resolve its deps), so fleet doctrine does not apply, exactly as it does not
 * under `upstream/` or `vendor/`; the walker cannot catch them by dir NAME
 * because `deps` alone is too generic to skip globally.
 */
export function isVendoredOverlayPath(relPath: string): boolean {
  return normalizePath(relPath).includes('source-patched/deps/')
}
...
export function scanSource(
  ...
  // Vendored upstream staged into a patched-source overlay: not fleet source,
  // full skip — the same posture the walker takes for `upstream/` and `vendor/`.
  if (isVendoredOverlayPath(relPath)) {
    return out
  }
  const inPortTree = isUnderAny(relPath, opts.portRoots ?? [])
  const capExempt =
    inPortTree || hasLineCapPragma(lines) || isGeneratedSource(lines)
  if (lines.length > MAX_SOURCE_LINES && !capExempt) {
    out.push({
      file: relPath,
      line: lines.length,
      rule: 'max-file-lines',
