 ...
 *   green.
 *
 *   Reads the local `.config/socket-wheelhouse.json` `bundle` block. A repo with
 *   no pin (the wheelhouse-as-producer, a non-thin member) passes vacuously.
 *   When a pin exists, resolves the release at `bundle.ref` via gh and asserts
 *   `templateSha === bundle.cascadeSha`. NETWORK-GATED like the other registry
 *   checks: when gh is unavailable (no binary / not authenticated / offline),
 *   it SKIPS with a clear note rather than false-failing — the fetch-path verify
 *   still hard-fails at install time, so the invariant is never unguarded.
 *
 *   Usage: node scripts/fleet/check/release-and-cascade-are-paired.mts [--quiet]
 */
 ...
 * fails) when not — the fetch-path verify still guards the invariant at install.
 */
