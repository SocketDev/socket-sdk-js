 ...
 *   runtime for every member that installs from the manifest but never gets
 *   the transitive package on disk — the incident this check exists to catch:
 ...
 *   installed.
 *
 *   Scans every `.mts` file under `.claude/hooks/fleet/` (always) and
 *   `.claude/hooks/repo/` (when present — not every fleet member carries one).
 *   Extracts every STATIC bare-specifier import: `import … from '<spec>'`,
 *   the bare side-effect form `import '<spec>'`, and `export … from '<spec>'`
 *   re-exports — including `import type` / `export type` (still needs the
 *   package on disk for `tsc` to resolve its types). Dynamic `import(...)` and
 *   `require(...)` are not static and are never matched. A `node:*` builtin or
 *   a relative (`./`, `../`) specifier names no installed package and is
 *   skipped. A subpath import (e.g. `@socketsecurity/lib-stable/logger/default`)
 *   resolves to its package name (`@socketsecurity/lib-stable`) before the
 *   declared-deps check.
 *
 *   Fails loud (What / Where / Saw / Wanted / Fix) listing every undeclared
 *   specifier + the importing file — never a silent skip.
 *
 *   Usage: node scripts/fleet/check/hook-imports-are-declared.mts [--quiet]
 */
 ...
 * passing.
 */
export function readDeclaredPackageNames(
...
export function findUndeclaredImports(
  ...
  findings.sort(
    (a, b) =>
      a.file.localeCompare(b.file) || a.packageName.localeCompare(b.packageName),
  )
  return findings
}
