...
export function findUnsetFleetEnv(
...
async function main(): Promise<void> {
  ...
  logger.log('  Fix (dev):  node .claude/hooks/fleet/setup-security-tools/install.mts')
  logger.log('  Fix (CI):   the reusable CI workflow sets FLEET_ENV in its env:.')
  process.exitCode = 1
}
