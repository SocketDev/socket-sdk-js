...
function main(): void {
  ...
  logger.log('Building the hook compile-cache bundle + snapshot blob + launcher…')
  ...
  const launcherBin = path.join(DISPATCH_DIR, isWin ? 'dispatch-launcher.exe' : 'dispatch-launcher')
...
export { baselineCommand, isDispatchCommand, launcherCommand, rewriteDispatchCommands }
