#!/usr/bin/env node
/**
 * @fileoverview Generates docs/api-reference.md from src/socket-sdk-class.ts and
 ...
 *
 * Usage:
 *   node scripts/generate-api-reference.mts          # write the file
 *   node scripts/generate-api-reference.mts --check  # diff only, exit 1 on drift
 */
...
const outPath = path.join(rootPath, 'docs/api-reference.md')
...
function main(): void {
  ...
  if (check) {
    ...
    if (current !== next) {
      logger.error(
        `docs/api-reference.md is out of date. Run: pnpm run docs:api`,
      )
      process.exitCode = 1
      return
    }
    logger.success('docs/api-reference.md is up to date')
    return
  }
...
export function render(methods: MethodInfo[]): string {
  ...
  sections.push('  Source: scripts/generate-api-reference.mts')
  sections.push('-->')
  sections.push('')
  ...
  if (otherMethods.length > 0) {
    ...
    sections.push('')
    sections.push(
      'Methods not yet placed into a domain group. Add them to `GROUPS` in `scripts/generate-api-reference.mts`.',
    )
    sections.push('')
