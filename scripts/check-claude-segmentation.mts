/**
 * @file Enforce `.claude/{agents,commands,hooks,skills}/` segmentation.
 *
 *   Every entry in those four directories must live under `fleet/<name>/` (when
 *   the wheelhouse template ships an entry with that name) or `repo/<name>/`
 ...
 *   should be removed or rehomed.
 *
 *   Why this matters: the wheelhouse cascade synth + hooks all key on the
 *   `fleet/`-prefixed shape. Dangling top-level entries duplicate or shadow the
 *   canonical copy, breaking skill/command/agent/hook resolution in
 *   unpredictable ways.
 *
 *   Past incident: 2026-06-01 fleet-wide audit found ~200 dangling entries
 *   across 10 repos — every fleet repo had at least 18 duplicate top-level
 *   skill directories shadowing their `fleet/<name>/` counterparts.
 *
 *   Exceptions: `_shared/` (and any other `_`-prefixed name) is allowed at
 *   the top level — it's the documented internals folder.
 *
 *   Behavior:
 *
 *   - Read mode (default): exit 1 with a per-entry report when dangling
 *     entries exist. Exit 0 when clean.
 ...
 *     have a `fleet/<name>/` counterpart.
 *
 *   The wheelhouse template's fleet set is read from
 *   `<wheelhouse>/template/.claude/<kind>/fleet/` at runtime when invoked
 *   from a fleet repo. In a fleet repo without a sibling wheelhouse
 *   checkout, the script falls back to a built-in list (kept in lockstep
 *   with the template via the wheelhouse cascade itself).
 */
 ...
 * `socket-wheelhouse/template/.claude/<kind>/fleet/` checkout first; falls
 * back to the BUILTIN_FLEET_SET below if the wheelhouse isn't reachable
 * (e.g. in CI where the fleet repo is checked out alone).
 */
export function getFleetSet(kind: string): Set<string> {
  const candidates = [
    path.join(REPO_ROOT, '..', 'socket-wheelhouse', 'template', '.claude', kind, 'fleet'),
    path.join(REPO_ROOT, '..', '..', 'socket-wheelhouse', 'template', '.claude', kind, 'fleet'),
  ]
 ...
 * re-syncs this file too. If you're editing this list by hand, you're
 * probably in the wrong place; add to the wheelhouse template instead.
 *
...
/**
 * Apply the fix for each dangling entry — `rm -rf` for `dup-of-fleet`,
 * `mv` for `rehome-to-fleet` / `move-to-repo`. Operates on the filesystem;
 * commit + push is the caller's job.
 */
...
async function main(): Promise<void> {
  ...
  logger.log(
    `[check-claude-segmentation] Applying ${entries.length} fix(es):`,
  )
