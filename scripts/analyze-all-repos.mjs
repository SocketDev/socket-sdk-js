...
function analyzeRepo(repoName) {
  ...
  const hasEsbuildConfig =
    existsSync(path.join(repoPath, '.config', 'esbuild.config.mjs')) ||
    existsSync(path.join(repoPath, 'esbuild.config.mjs'))

  // Check for rollup config
  const hasRollupConfig =
    existsSync(path.join(repoPath, '.config', 'rollup.config.mjs')) ||
...
function printAnalysis(analysis) {
  ...
  for (const repo of analysis) {
    ...
    if (repo.hasEsbuild) {
      logger.log(
        `    ✅ esbuild${repo.hasEsbuildConfig ? ' (with config)' : ''}`,
      )
    }
    if (repo.hasRollup) {
      logger.log(`    ✅ rollup${repo.hasRollupConfig ? ' (with config)' : ''}`)
    }
    if (repo.hasVite) {
      logger.log('    ✅ vite')
    }
    if (!repo.hasEsbuild && !repo.hasRollup && !repo.hasVite) {
      logger.log('    ❌ No bundler found')
    }
  ...
  const compatible = analysis.filter(
    r => r.exists && r.hasPackageJson && r.hasEsbuild,
  )
  const needsSetup = analysis.filter(
    r => r.exists && r.hasPackageJson && !r.hasEsbuild,
  )
  ...
  for (const repo of compatible) {
    logger.log(
      `  ✅ ${repo.name} - ${repo.hasEsbuildConfig ? 'Has config' : 'Needs config'}`,
    )
  }
  logger.log('')
  ...
  for (let i = 0; i < sorted.length; i++) {
    const repo = sorted[i]
    logger.log(
      `  ${i + 1}. ${repo.name} (${repo.entryCount} entries) ${repo.isMonorepo ? '[MONOREPO]' : ''}`,
    )
  }
}
