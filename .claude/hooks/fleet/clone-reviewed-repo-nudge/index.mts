/**
 * @file Claude Code PreToolUse hook — clone-reviewed-repo-nudge. When an agent
 *   reviews or references an EXTERNAL GitHub repo (one that is not a SocketDev
 *   fleet member), it should clone the repo locally so it can `grep` / read /
 *   index the tree — rather than reading it only through the GitHub web/API a
 *   file at a time. The fleet standardizes both WHERE the clone lands and HOW
 *   small it is: Where: ~/.socket/_wheelhouse/repo-clones/<org>-<repo>/
 *   (lowercased + dash-cased; resolve via getSocketRepoClonesDir()). NEVER
 *   ~/projects/* — the fleet's sibling-walk tooling treats those as member
 *   checkouts. How: git clone --depth=1 --single-branch --filter=blob:none
 *   <url> <dest> (shallow + single-branch + blobless partial = smallest
 *   practical footprint and fastest download; blobs fetched lazily on access).
 *   Two nudge arms, both stderr-only (this is a -nudge: it never blocks): (1)
 *   Reviewing an external repo through `gh` (`gh repo view <owner/repo>`, `gh
...
export const check = bashGuard(command => {
  ...
  for (const cmd of commandsFor(command, 'git')) {
    ...
    if (result && result.missing.length) {
      return notify(
        buildMissingFlagsMessage(result.owner, result.repo, result.missing),
      )
    }
  }
  return undefined
