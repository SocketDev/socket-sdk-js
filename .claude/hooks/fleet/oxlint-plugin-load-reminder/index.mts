#!/usr/bin/env node
// Claude Code PostToolUse hook — oxlint-plugin-load-reminder.
//
...
await withEditGuard(filePath => {
  ...
  if (result.status !== 0) {
    logger.error(
      `🚨 oxlint-plugin-load-reminder: the socket/ oxlint plugin no longer loads cleanly after editing ${filePath}. Every socket/ rule is disabled until this is fixed. Details above (from check-oxlint-plugin-loads.mts); run \`node scripts/fleet/check/oxlint-plugin-loads.mts\` to re-check.`,
    )
