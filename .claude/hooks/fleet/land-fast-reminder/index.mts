#!/usr/bin/env node
// Claude Code Stop hook — land-fast-reminder.
//
...
// ahead-only main is the unpushed-main-reminder's job (just push); a
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    [
      `[land-fast-reminder] ${branch} has DIVERGED from origin/${branch}: ` +
...
if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
  main().catch(e => {
    ...
    process.stderr.write(
      `land-fast-reminder: hook error (continuing): ${(e as Error).message}\n`,
    )
  })
}
