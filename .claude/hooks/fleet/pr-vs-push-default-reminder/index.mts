...
await withBashGuard((command, payload) => {
  ...
  if (isPush && !isPrCreate) {
    ...
    logger.error(
      [
        '[pr-vs-push-default-reminder] Pushing a feature branch in a fleet repo',
        '',
  ...
  logger.error(
    [
      onDefault
        ? '[pr-vs-push-default-reminder] About to open a PR from the default branch'
        : '[pr-vs-push-default-reminder] About to open a PR from a fleet feature branch',
      '',
      `  Branch: ${branch}   Repo: ${slug ?? '(unknown)'}   Default: ${defaultBranch}`,
