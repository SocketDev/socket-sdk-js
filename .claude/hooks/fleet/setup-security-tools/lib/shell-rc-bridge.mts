 ...
 * memory: feedback_keychain_prompts.md).
 *
 ...
 *
 * Idempotent: a second call with the same token rewrites the block in place
 * rather than appending a duplicate. Different tokens trigger a rewrite. The
 * block is matched by BLOCK_BEGIN / BLOCK_END sentinels so it's safe to share
 * an rc with other managed blocks (homebrew, nvm, etc.).
 */
