...
test('CLI: Write CLAUDE.md with new long no-link section warns (exit 0)', () => {
  ...
  try {
    ...
    assert.match(stderr, /prefer-external-detail/)
...
test('CLI: disabled env var short-circuits', () => {
  const tmpdir = mkdtempSync(path.join(os.tmpdir(), 'cmped-'))
  try {
    const filePath = path.join(tmpdir, 'CLAUDE.md')
    writeFileSync(filePath, fleetDoc('### Old\nshort'))
    const newContent = fleetDoc(
      '### Old\nshort\n\n### New Section\nLine 1.\nLine 2.\nLine 3. no link.',
    )
    const result = spawnSync('node', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        SOCKET_CLAUDE_MD_PREFER_EXTERNAL_DETAIL_DISABLED: '1',
      },
    })
