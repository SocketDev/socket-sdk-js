...
out-of-band case: a dependency or upstream wrote the files. Edit-time hooks
can't see that kind of write. Distinct base names so they don't violate the
fleet `-guard`/`-nudge` no-overlap rule.
