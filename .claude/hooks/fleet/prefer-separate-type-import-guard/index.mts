#!/usr/bin/env node
// Claude Code PreToolUse hook — prefer-separate-type-import-guard.
//
...
// + hook + lint, same as prefer-function-declaration-guard). Across the fleet,
...
await withEditGuard((filePath, content, payload) => {
  if (process.env['SOCKET_PREFER_SEPARATE_TYPE_IMPORT_GUARD_DISABLED']) {
    return
  }
  ...
  if (bypassPhrasePresent(payload.transcript_path, [BYPASS_PHRASE])) {
    logger.error(
      `prefer-separate-type-import-guard: ${count} inline type specifier(s) — bypassed via "${BYPASS_PHRASE}"\n`,
    )
    return
  }
  ...
  logger.error(
    [
      `[prefer-separate-type-import-guard] ${count} inline \`type\` specifier(s) in a value import.`,
      '',
