#!/usr/bin/env node
// Claude Code PreToolUse hook — synthesized-script-edit-reminder.
//
...
// hand-edit to one of those `scripts` entries in package.json gets clobbered by
// the next `chore(wheelhouse): cascade …` — the manifest is the source of truth.
//
// Past incident (2026-06-06): renaming a check left doctor:auth in
// CANONICAL_SCRIPT_BODIES pointing at a deleted file; the fix had to land in the
// manifest, not package.json — but editing package.json directly silently
// reverted on the next cascade.
//
// This hook NUDGES (never blocks — exit 0): when an Edit/Write to a
// `package.json` touches a `scripts` key that the manifest synthesizes, it
// points you at the manifest. Only fires in the wheelhouse (the only repo that
// ships the manifest); in a cascaded fleet repo the manifest is absent and the
// hook is a no-op.
//
// Exit codes:
//   0 — always. Informational reminder; the edit proceeds.
...
export async function main(): Promise<void> {
  await withEditGuard((filePath, content) => {
    ...
    process.stderr.write(
      [
        `[synthesized-script-edit-reminder] This package.json edit touches a cascade-synthesized script:`,
        '',
