// node --test specs for the synthesized-script-edit-reminder hook.
//
// PreToolUse reminder scoped to package.json Edit/Write. NEVER blocks (exit 0);
// nudges to stderr when the edit touches a `scripts` key that the cascade
// synthesizes from CANONICAL_SCRIPT_BODIES in the manifest. Wheelhouse-only: no
// manifest downstream → silent. No bypass phrase, no env kill switch.
...
test('e2e: nudges (exit 0) on a package.json edit touching a synthesized key', async () => {
  ...
  assert.equal(code, 0)
  assert.match(stderr, /synthesized-script-edit-reminder/)
  assert.match(stderr, /doctor:auth/)
})

test('e2e: silent on a package.json edit touching only a NON-synthesized key', async () => {
  const repo = fixtureRepo()
  const { code, stderr } = await runHook(
    {
      tool_name: 'Edit',
