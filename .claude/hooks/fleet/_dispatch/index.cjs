...
// for the live .claude/hooks/fleet/_dispatch/ copy, but the identical
// template/base/ SEED copy resolves 4-up to `template/base/` — so when a test or
...
try {
  ...
  if (repoRoot) {
    require('node:module').enableCompileCache?.(
      path.join(repoRoot, 'node_modules', '.cache', 'fleet-hooks'),
    )
  }
} catch {
...
// MUST precede the first non-builtin require. Keep `./bundle.cjs` — and every
