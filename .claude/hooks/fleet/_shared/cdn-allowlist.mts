...
// Hosts the fleet actually curl/wget-fetches in CI / Docker / build setups, or
// that the gh CLI surface needs. Curated to real, evidenced need — NOT a
// blanket registry dump (package installs go through pnpm/uv/cargo, which the
...
// Expand only with an evidenced fetch need — less is more.
...
export function findDisallowedCdn(
  command: string,
): DisallowedCdnHit | undefined {
