...
export interface DispatchHookEntry {
   ...
   * `undefined`. Mutually exclusive with `check` — exactly one is set per
   * entry. No current hook uses it — every bundled hook is a `defineHook`
   * `check`; the seam stays for a future legacy port.
   */
   ...
   */
  readonly check?:
    | ((payload: DispatchPayload) => DispatchVerdict | Promise<DispatchVerdict>)
    | undefined
}

/**
 * The single-hook addressing protocol version. The literal is embedded in the
 * built bundle (`_dist/fleet-pack.cjs`), so a name-scoped caller — the
 * user-global `~/.claude/hooks/wheelhouse-dispatch.mts` — can grep the bundle
 * bytes to learn the bundle understands `index.cjs <Event> <hook-name>` before
 * relying on it. An older bundle without the literal would IGNORE argv[3] and
 * run the whole event's hook set, so the capability probe is load-bearing.
 */
...
/**
 * Whether a manifest matcher (a `|`-joined tool list, e.g. `Bash|Edit`) covers
 * `tool`. An empty matcher covers every tool.
 */
