/**
 * @file The entrypoint guard for a "legacy" fleet hook — one that exports a pure
 *   helper (e.g. `run`) AND self-executes a `main()` when invoked directly. The
 *   idiom every such hook hand-rolled was:
 *
 *     if (process.argv[1] && import.meta.url === `file://${process.argv[1]}`) {
 *       main().catch(() => process.exit(0))
 *     }
 *
 *   That bare check has a snapshot-build hazard. The hook-dispatch SNAPSHOT
 *   bundle imports each bundled hook's pure helper, so the hook's whole module —
 *   including this guard — lands in the bundle's top-level eval graph. When
 ...
 *   absolute file URL — so the equality holds, `main()` fires DURING THE BUILD
 *   PASS, hits empty stdin, and `process.exit(0)`s, which silently aborts
 *   serialization (exit 0, NO blob, only the `node:module` warning). A RELATIVE
 *   entry path masks it (bare `argv[1]` ≠ absolute URL), so the bug is
 *   path-form-dependent and easy to miss.
 *
 ...
 *   circuit so the guard is inert inside the snapshot builder. `defineHook` hooks
 *   get the equivalent gating from `isGuardRunContext` in guard.mts; this helper
 *   is for the legacy pure-`run` hooks that don't go through `runGuard`.
 */
