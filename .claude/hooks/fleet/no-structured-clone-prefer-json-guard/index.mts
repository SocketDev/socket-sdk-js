#!/usr/bin/env node
// Claude Code PreToolUse hook — no-structured-clone-prefer-json-guard.
//
...
const ALLOW_MARKER = '// socket-hook: allow structured-clone'
 ...
 * the older `// socket-hook: allow structured-clone` shape (same-line or
 * preceding-line).
 */
  ...
  process.stdin.on('end', () => {
    try {
      ...
      process.stderr.write(
        `[no-structured-clone-prefer-json-guard] refusing edit: ` +
      ...
      process.stderr.write(
        `[no-structured-clone-prefer-json-guard] hook error (allowing): ${e}\n`,
      )
      process.exit(0)
    }
  })
