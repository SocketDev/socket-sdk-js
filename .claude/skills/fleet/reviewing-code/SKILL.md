...
4. **`phase('Verify')`** - adversarial pass. Per High/Critical finding, spawn a skeptic `agent()`, the verify backend, default `claude`, that tries to REFUTE the finding against the actual diff, returning a `VERDICT_SCHEMA` (`{ isReal, verdict: confirmed|likely|false-positive, why }`). Discard findings the skeptic refutes before they land in the report; mark survivors `CONFIRMED`. Skip this phase when `--skip-verify` is set and `log()` that the report is unverified.
