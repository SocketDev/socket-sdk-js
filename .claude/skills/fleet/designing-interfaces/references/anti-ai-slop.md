...
## 🚨 THE #8 TELL: CATEGORY REFLEXES

The first reflex turns a product category into a familiar aesthetic: the dark terminal for a
developer tool, the soft blue dashboard for finance, the warm serif editorial page for any
brand. The second reflex avoids that first choice but lands on its equally familiar opposite.

**RULE: Test the direction twice.** Ask whether someone could infer the visual family from
the product category alone. Then ask whether they could infer it from the category plus the
obvious anti-reference. Rework either answer until the product brief and research explain the
direction better than the category does.

---

## 🚨 THE #9 TELL: DECORATIVE GRID BACKGROUNDS

Two hairline gradient layers with a repeated background size produce a grid overlay that now
appears on generated empty surfaces by default.

**RULE: Use a grid only when it carries product meaning.** Canvases, maps, blueprints, and
measurement tools may need one. Other pages need product structure, media, hierarchy, or a
plain surface.

---

## 🚨 THE #10 TELL: TOKEN ROLE DRIFT
...
□ Neither category reflex nor its predictable anti-default determined the visual direction
□ No decorative gradient grid background without a product-specific purpose
