...
| # | Phase | Outcome |
| --- | --- | --- |
| 1 | Promote | `node scripts/check-soak-exclude-dates.mts --fix` removes every entry whose `removable:` date has passed (the bullet + its `# published/removable` annotation). |
...
```bash
node scripts/check-soak-exclude-dates.mts --fix
