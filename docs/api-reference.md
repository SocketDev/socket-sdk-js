...
Every API method returns a *result object*, not a raw response. You always check `success` first:
...
2. **Unreadable files don't crash by default.** If a file is missing or unreadable, the SDK logs a warning and continues with the rest. If *every* file is unreadable, it throws. Override with `onFileValidation`:
