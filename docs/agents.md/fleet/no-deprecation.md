...
- **Delete and rewire in the same change.** Replacing `foo` with
...
- **No `@deprecated` / `@obsolete` markers.** A deprecation annotation is a
  promise to delete later that never gets kept, so it accretes. If it's deprecated,
  it's deletable now.
...
consumers to protect with a migration window. It cascades. So the right move is
