...
2. **Add a per-line marker** if the rule has one (e.g. `// oxlint-disable-next-line socket/no-console-prefer-logger` for the console-prefer-logger rule). Documents the exemption inline.
