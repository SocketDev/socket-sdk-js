...
| Hook bypass attempts             | `.claude/hooks/fleet/no-revert-guard/`                  | Blocks `git revert`, `--no-verify`, `HUSKY=0`, a redirected `core.hooksPath`, `--no-gpg-sign` — gated by canonical `Allow X bypass` phrases                                                                                                    |
