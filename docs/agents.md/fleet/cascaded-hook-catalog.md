...
- **commit-message-format-guard** - PreToolUse(Bash) enforcing Conventional Commits
  and banning AI attribution. Bypass: `Allow commit-format bypass` / `Allow ai-attribution bypass`.
...
entry (per-file entries redundant, since the rm-and-copy dir mirror covers them).
