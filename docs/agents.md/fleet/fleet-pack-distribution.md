...
- **Belt (dev / clone)** - `package.json` `prepare` starts with
  ...
  recorded by a local marker at `.cache/fleet/socket-wheelhouse/bundle-applied`,
...
itself (the fetcher can't ship inside the bundle it fetches, marked `releaseExclude`
...
cascade. The module THROWS at load time on any overlap. A RETIRED mirror
