...
The `token-spend-guard` hook nudges when a mechanical command runs on a premium model or high effort. Note: that covers a cascade, an autofix sweep, and a bulk rename. Treat the nudge as a signal to drop down a tier before continuing.
