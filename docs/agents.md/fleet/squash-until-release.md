...
- **SHA pins.** A `git+https://…#<sha>` dependency, a `.gitmodules` pin, and a
  workflow `uses: owner/repo@<sha>` each resolve a commit that must keep
  existing.
...
either way, because `npm audit signatures` verifies the attestation's signature,
not whether the commit it names is still reachable. That makes "provenance
breaks" narrower than it first sounds. The pins and the source link are what
actually dangle, and freezing the release commit costs nothing to avoid, so the
freeze happens regardless.
...
- Every candidate is accepted only when `git merge-base --is-ancestor <sha>
  ...
  REJECTED.
- A multi-package or multi-crate member can carry several release anchors. The
  boundary is the NEWEST one across all of them that passes the ancestor
  check.
