...
## 2. `new URL(…).pathname` yields `/D:/…` — drive doubling
...
`spawnSync(process.execPath, [binPath, …args])` — identical on unix, the only
...
- Case-insensitive filesystem — `Foo.ts` and `foo.ts` collide; a case-sensitive
