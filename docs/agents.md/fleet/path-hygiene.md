...
- **Beyond build paths**: `paths.mts` covers config files, lockfiles, cache dirs, and manifest files. The fleet ships a starter `template/scripts/paths.mts` that exports the common constants + `loadFleetConfig()`.
