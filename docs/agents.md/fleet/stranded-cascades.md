...
The script lives at `scripts/repo/cleanup-stranded.mts`. You don't normally run it directly — the cascade does that — but it's safe to invoke ad-hoc:

```bash
# Dry-run against one repo (substitute the actual repo path).
node $PROJECTS/scripts/repo/cleanup-stranded.mts \
...
# Apply the fix.
node $PROJECTS/scripts/repo/cleanup-stranded.mts --all
```
...
2. **Cascade commit touching unexpected files**: inspect with `git show <sha>`. If the cascade should have written that path, lift the path into the cascade allowlist (in `scripts/repo/cleanup-stranded.mts`) and re-run. If the file shouldn't be cascade-touched at all, this is an authoring bug in `scripts/repo/sync-scaffolding/manifest.mts`.
...
- It does **not** sync the cascade's actual content. That's `scripts/repo/sync-scaffolding/cli.mts`'s job.
