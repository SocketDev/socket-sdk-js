...
| ------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| npm           | package.json + pnpm-lock.yaml                        | pnpm `minimumReleaseAge` (minutes) + `.npmrc` `min-release-age`                                                                                                                    | pnpm rejects unsoaked at install; parity: `scripts/fleet/check/soak-time-is-consistent.mts`                                                                                                                                            |
| python        | pyproject.toml + uv.lock                             | `[tool.uv] exclude-newer`                                                                                                                                                          | uv rejects newer; `scripts/fleet/check/uv-lockfiles-are-current.mts`                                                                                                                                                                   |
| rust          | Cargo.toml + Cargo.lock                              | `.cargo/config.toml` min-publish-age (nightly-native; inert on stable, where the LOCK is build-time enforcement and the nightly updater `scripts/fleet/update/cargo.mts` is the only lock-mover) | `scripts/fleet/check/cargo-soak-config-is-current.mts`: config parity unconditional, tracked Cargo.lock required with own rust. Repo-only Cargo settings live in optional `.cargo/config.repo.toml`, included by the canonical config. |
...
- Prebaked images: every `docker/fleet/*.Dockerfile` copies its lock
  ...
  `scripts/repo/check/prebakes-install-from-lock.mts` (repo tier) fails an unpinned
