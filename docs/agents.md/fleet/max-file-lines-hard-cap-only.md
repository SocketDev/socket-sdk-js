...
When a file legitimately exceeds 1000 lines, the marker is allowed. Note: that covers a generated artifact, a spec table, or a genuine single cohesive unit. The category field must name WHAT the file is (`generated`, `spec-table`, `registry`), not a meta-label like `ok` or `exempt`. The reason field must state WHY it cannot be split.
