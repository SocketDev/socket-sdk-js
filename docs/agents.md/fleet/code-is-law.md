...
- **Lint rule** (`socket/<rule>` in `template/base/.config/fleet/oxlint-plugin/`): commit-time and editor enforcement, for violations visible in source text or AST. Default `"error"` (never `"warn"`). Ship an autofix (`fixable: 'code'`) when the rewrite is deterministic.
...
- **DRY into `_shared/`.** When a hook and a check script (or two hooks) need the same detection logic, lift it into a `.claude/skills/fleet/_shared/` lib (or `_shared/scripts/`) and import it. Note: that logic can be a parser, a regex set, an allowlist, or a shell-command AST walk. Copy-pasted detection drifts: one copy gets the fix, the other stays buggy, and a green gate hides the gap.
