...
makes a reference's role unmistakable, because the path itself says "this is
upstream code we pin." It also keeps the ignore + gitlink rules a single path
...
- **`scripts/fleet/vendor-actions.mts`** vendors every ported-from upstream
  (any `<owner>/<repo>`, not only `actions/*`) as an `upstream/<owner>-<repo>`
...
upstream release surfaces in the weekly PR as an actionable advisory: re-pin,
re-review, bump `portedAt`.
...
- `scripts/fleet/check/action-ports-are-lock-stepped.mts` fails when a
  ...
  differs from the pinned tag. That is the lock-step gate described above.
