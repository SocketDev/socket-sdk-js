...
describe('SocketSdk - downloadPatch', () => {
  ...
  it('should download patch with SSRI hash format', async () => {
    ...
    const result = await getClient().downloadPatch(ssriHash, { baseUrl: getBaseUrl() })
  ...
  it('should download patch with hex hash format', async () => {
    ...
    const result = await getClient().downloadPatch(hexHash, { baseUrl: getBaseUrl() })
  ...
  it('should throw error when patch not found (404)', async () => {
    ...
    await expect(getClient().downloadPatch(hash, { baseUrl: getBaseUrl() })).rejects.toThrow(
  ...
  it('should properly encode special characters in hash', async () => {
    ...
    const result = await getClient().downloadPatch(hashWithSpecialChars, { baseUrl: getBaseUrl() })
  ...
  it('should download large patch content', async () => {
    ...
    const result = await getClient().downloadPatch(hash, { baseUrl: getBaseUrl() })
