 ...
 *   sites. Bypass: a `oxlint-disable-next-line socket/prefer-typebox-schema`
 *   comment on the import (rare — e.g. a test fixture that must reproduce a
 *   zod-specific bug).
 */
...
const rule = {
  meta: {
    ...
    messages: {
      banned:
        '`{{pkg}}` — the fleet standardizes on @sinclair/typebox for runtime schema validation (Type.Object({…})). A second validation runtime fragments the fleet; port the schema to TypeBox. Bypass: add a `oxlint-disable-next-line socket/prefer-typebox-schema` comment if this import is genuinely required.',
    },
    schema: [],
  },
