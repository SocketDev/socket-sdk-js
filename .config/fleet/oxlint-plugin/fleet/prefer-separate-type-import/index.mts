    ...
    return {
      ImportDeclaration(node: AstNode) {
        ...
        const typeNames = typeSpecifiers
          .map((s: AstNode) =>
            specifierText(sourceCode, s, { stripType: true }),
          )
          .join(', ')
        ...
        for (let i = 0, { length } = typeSpecifiers; i < length; i += 1) {
          ...
          const name = spec.imported?.name ? spec.imported.name : '<unknown>'
          const report: {
            node: AstNode
            messageId: string
            ...
            report.fix = function (fixer: RuleFixer) {
              ...
              if (valueSpecifiers.length > 0) {
                const valueText = valueSpecifiers
                  .map((s: AstNode) =>
                    specifierText(sourceCode, s, { stripType: false }),
                  )
                  .join(', ')
