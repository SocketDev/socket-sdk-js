  ...
  create(context: RuleContext) {
    ...
    return {
      ...
      Identifier(node: AstNode) {
        ...
        // TS type/interface members, class members, and enum members put the
        // name in declaration position — `hash: string` in a type literal or
        // `sign() {}` in a class is not a value reference, and rewriting one
        // produces invalid syntax (`crypto.hash: string`).
        if (
