...
function rewriteHit(match: string): string | undefined {
  ...
  /* c8 ignore start - unreachable: rewriteHit is only called for stems confirmed in SUBSTITUTIONS */
  return undefined
  /* c8 ignore stop */
}
      ...
      if (!mutated) {
        ...
        for (let i = 0, { length } = hits; i < length; i += 1) {
          ...
          if (h.stem === 'master') {
            ...
            /* c8 ignore start - unreachable: !mutated loop only contains REPORT_ONLY (master/slave) hits; else-if false-arm never fires */
