...
// Matches `.cache/` as a path segment at the start, mid-path, or as a trailing `/.cache` — but NOT a bare `.cache` string.
...
const rule = {
  ...
  create(context: RuleContext) {
    ...
    function isHomeDirExpression(node: AstNode) {
      /* c8 ignore start - defensive null guard; AST parsers never produce null nodes in arguments arrays */
      if (!node) {
        return false
      }
      /* c8 ignore stop */
