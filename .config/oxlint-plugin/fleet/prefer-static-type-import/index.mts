/**
 * @file Flag inline `import('module').Name` type expressions — use a static
 *   `import type { Name } from 'module'` at the top of the file instead.
 *   Inline-import type expressions read worse than the static form for three
 *   reasons:
 *
 *   1. Repeat usages duplicate the module path at every annotation site, so
 *      renaming the module is a multi-edit instead of a one-line header
 *      change.
 *   2. The reader has to parse the type expression to discover what's imported; a
 ...
 *     import expression). Captures the module specifier plus the qualifier (the
 *     property name read off the imported namespace). No autofix:
 *   - Adding a static `import type` requires choosing a unique local name and
 *     inserting at the correct sort position. The fleet's `sort-named-imports`
 *     + `prefer-separate-type-import` rules already enforce the import-header
 *     shape; rather than racing them with a half-built rewrite, this rule
 *     reports the violation and leaves the lift to the human (one-line edit
 *     anyway). Allowed exceptions (skipped — no report):
 *   - `typeof import('module')` namespace forms (TSImportType wrapped in
 *     TSTypeQuery). The static equivalent is `import * as Foo from 'module'`
 *     followed by `typeof Foo`, which is heavier than the inline form for
 *     one-shot uses. Why a rule and not just a code-style note: socket-lib
 *     drift incident 2026-05-14 — `SpawnOptions` accumulated inline-import
 *     properties (`spinner?: import('../spinner/types').Spinner`) over time.
 *     When the type was extended for a sibling `NodeSpawnSyncOptions`, the
 *     inline shape duplicated the same module path again. A static `import type
 *     { Spinner } from '../spinner/types'` makes the extension a no-edit at the
 *     type-spec level.
 */
