 ...
 *   backtick-split form). This rule flags any block comment whose prose contains
 *   a star-immediately-before-slash sequence (escaped backslash-slash included)
 *   and autofixes it to the backtick-split form. Line comments are exempt — they
 *   have no closing token to break. The comment's own trailing close token is
 *   not matched (it is the close, not prose).
 */
