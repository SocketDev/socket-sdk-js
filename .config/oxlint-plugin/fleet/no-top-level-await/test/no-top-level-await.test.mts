    ...
    new RuleTester().run('no-top-level-await', rule, {
      ...
      invalid: [
        ...
        },
        {
          ...
          // `.mts` so oxlint parses it as an ES module: `for await` at module
          // scope only carries the `ForOfStatement.await` flag (and is real TLA)
          // in a module context. In a plain `.ts` script oxlint drops the flag,
          // so the default `fixture.ts` would silently not flag this.
