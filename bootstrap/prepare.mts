 ...
 *      (delegates to `bootstrap/fleet.mts --if-current`). On a fresh clone this
...
export function fetchBundle(): void {
  const fleet = path.join(HERE, 'fleet.mts')
  if (!existsSync(fleet)) {
    log('no bootstrap/fleet.mts beside me — skipping bundle fetch')
    return
  }
  if (!tryRun('node', [fleet, '--if-current'])) {
    log('bundle fetch (fleet.mts --if-current) reported a problem — continuing')
  }
}
